...
class DbBackup(models.Model):
    ...
    tmp_dir = fields.Char('Temporary Directory', help='Absolute path of temporary directory')
    ...
    email_to_notify = fields.Char('E-mail to notify',
                                  help='Fill in the e-mail where you want to be notified that the backup failed on '
                                       'the FTP.')
    is_debug_log = fields.Boolean("Enable debug messages")
    ...
    def _debug_log(self, message):
        if self.is_debug_log:
            self.message_post(body=message, subtype="mail.mt_comment", message_type="comment")

    @api.model
    def schedule_backup(self):
        conf_ids = self.search([])
        for rec in conf_ids:
            rec.message_post(body="Backing-up database {}".format(rec.name),
                             subtype="mail.mt_comment",
                             message_type="comment")
            try:
                if not os.path.isdir(rec.folder):
                    os.makedirs(rec.folder)
                if rec.tmp_dir and not os.path.isdir(rec.tmp_dir):
                    os.makedirs(rec.tmp_dir)
            except:
                raise
            ...
            try:
                ...
                self._take_dump(rec.name, fp, rec.backup_type, rec.tmp_dir, rec._debug_log)
                fp.close()
                rec.message_post(body="Backed up database {} to {}".format(rec.name, file_path),
                                 subtype="mail.mt_comment",
                                 message_type="comment")
            except Exception as error:
                rec.message_post(body="Couldn't backup database {}. Bad database administrator password for server "
                                      "running at http://{}:{}\nError: {}".format(rec.name, rec.host, rec.port, error),
                                 subtype="mail.mt_comment",
                                 message_type="comment")
                continue
            ...
            if rec.sftp_write is True:
                try:
                    ...
                    try:
                        ...
                        rec.message_post(
                            body="Error connecting to remote server! {}".format(error),
                            subtype="mail.mt_comment",
                            message_type="comment")
                    try:
                            ...
                            if os.path.isfile(fullpath):
                                ...
                                # This means the file does not exist (remote) yet!
                                    ...
                                    try:
                                        ...
                                        rec.message_post(
                                            body="Couldn't write the file to the remote server. {}".format(err),
                                            subtype="mail.mt_comment",
                                            message_type="comment")
                    ...
                    try:
                        ...
                        pass
                    rec.message_post(
                        body="Couldn't back up to the FTP server. Here is what we got back instead: {}".format(e),
                        subtype="mail.mt_comment",
                        message_type="comment")
    ...
    def _take_dump(self, db_name, stream, backup_format, tmp_dir, debug_log_handler):
        ...
        debug_log_handler('DUMP DB: {} format {}'.format(db_name, backup_format))

        cmd = ['pg_dump', '--no-owner']
        cmd.append(db_name)

        if backup_format == 'zip':
            with tempdir(dir=tmp_dir or None) as dump_dir:
                debug_log_handler("Start filestore dump to {}".format(dump_dir))
                filestore = odoo.tools.config.filestore(db_name)
                if os.path.exists(filestore):
                    shutil.copytree(filestore, os.path.join(dump_dir, 'filestore'))
                debug_log_handler("Finished filestore dump.")
                with open(os.path.join(dump_dir, 'manifest.json'), 'w') as fh:
                    db = odoo.sql_db.db_connect(db_name)
                    with db.cursor() as cr:
                        json.dump(self._dump_db_manifest(cr), fh, indent=4)
                cmd.insert(-1, '--file=' + os.path.join(dump_dir, 'dump.sql'))
                debug_log_handler("Start DB dump: {}".format(cmd))
                odoo.tools.exec_pg_command(*cmd)
                debug_log_handler("Finished DB dump")
                if stream:
                    debug_log_handler("Start zip to stream")
                    odoo.tools.osutil.zip_dir(dump_dir, stream, include_dir=False,
                                              fnct_sort=lambda file_name: file_name != 'dump.sql')
                    debug_log_handler("Finished zip to stream")
                else:
                    debug_log_handler("Start zip to tempfile")
                    t = tempfile.TemporaryFile(dir=tmp_dir or None)
                    odoo.tools.osutil.zip_dir(dump_dir, t, include_dir=False,
                                              fnct_sort=lambda file_name: file_name != 'dump.sql')
                    t.seek(0)
                    debug_log_handler("Finished zip to tempfile")
                    return t
        else:
